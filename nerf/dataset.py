...
DEFAULT_DATA_ROOT = "data_lego_white"
...
def get_synthetic_datasets(
    ...
    data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", data_path)
    ...
    # train1 = range(40, 120)
    train1 = range(data_config["number_poses"])
...
def get_colmap_datasets(
    data_path: str = DEFAULT_DATA_ROOT,
    image_down_scale: int = 1,
    pose_down_scale: int = 1
    )-> Tuple[Dataset, Dataset, Dataset]:
        # Load eye and look at positions
    ...
    imgdir = os.path.join(os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", data_path, 'images'))
    ...
    focal_length = np.array(K_mat[0, 0])
    focal_lengths = torch.from_numpy(focal_length).expand(n_poses, 1).float()/image_down_scale
    principal_point = K_mat[0:2, 2]
    principal_points = torch.from_numpy(principal_point).expand(n_poses, 2).float()/image_down_scale
    image_size = [data_config["image_height"], data_config["image_width"]]
    image_sizes = torch.Tensor(image_size).expand(n_poses, 2)/image_down_scale

...
class WaterAlbedoField(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.color_layer = torch.nn.Sequential(
            torch.nn.Linear(input_dim, n_hidden_neurons_xyz),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_xyz, n_hidden_neurons_xyz),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_xyz, n_hidden_neurons_alb),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_alb, 3)
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # color = self.color_layer(torch.cat((features, embeds_world), dim=-1))
        color = self.color_layer(embeds_world)
        color = self.color_final_layer(color)
        return color

class WaterNormField(torch.nn.Module):
    def __init__(
        self,
        n_hidden_neurons_xyz: int = 256,
        n_hidden_neurons_nor: int = 128,
        input_dim: int = 32,
        **kwargs,
    ):
        """
        ...
        self.norm_layer = torch.nn.Sequential(
            torch.nn.Linear(input_dim, n_hidden_neurons_xyz),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_xyz, n_hidden_neurons_xyz),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_xyz, n_hidden_neurons_nor),
            torch.nn.LeakyReLU(True),
            torch.nn.Linear(n_hidden_neurons_nor, 3)
        )
        # _xavier_init(self.norm_layer)
    ...
    def forward(
        ...
        **kwargs,
        ...
        norm = self.norm_layer(embeds_world)
        norm = F.normalize(norm, dim = -1)
        return norm

class WaterDensityField(torch.nn.Module):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        print("++++++++++++++++++++++++++++++++")
        self.harmonic_embedding_xyz = HarmonicEmbedding(n_harmonic_functions_xyz, omega_0 = 0.5)
        ...
        self.medium_density = torch.nn.Parameter(torch.torch.rand(3), requires_grad=True)

        # self.density_layer = torch.nn.Linear(n_hidden_neurons_xyz, 1)
        self.density_layer = torch.nn.Linear(n_hidden_neurons_xyz, 3) # 3 channel for rgb
...
class WaterDensityFieldHash(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.hash_embedding_xyz = HashEmbedder()
        ...
        # Zero the bias of the density layer to avoid
        # a completely transparent initialization.
        self.density_layer.bias.data[:] = 0.5  # fixme: Sometimes this is not enough
    ...
    def _get_densities(
        ...
        depth_values: torch.Tensor,
        density_noise_std: float,
        water_raw_density: torch.Tensor,
    ) -> torch.Tensor:
        """
        This function takes `features` predicted by `self.mlp_xyz`
        and converts them to `raw_densities` with `self.density_layer`.
    ...
    def forward(
        ...
        **kwargs,
        ...
        raw_densities = self._get_densities(
                features, ray_bundle.lengths, density_noise_std, water_raw_density)
        raw_densities = self.softplus(raw_densities)
        return raw_densities, features, embeds_xyz
...
class MLPWithInputSkips(torch.nn.Module):
    ...
    def __init__(
        ...
        for layeri in range(n_layers):
            ...
            _xavier_init(linear)
            layers.append(torch.nn.Sequential(linear, torch.nn.LeakyReLU(True)))

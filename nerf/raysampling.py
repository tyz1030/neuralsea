...
class MultinomialRaysampler(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return _xy_to_bounded_ray_bundle(
            cameras,
            xy_grid,
            min_depth,
            max_depth,
...
def _xy_to_bounded_ray_bundle(
    ...
    # ray z-coords
    rays_zs = xy_grid.new_empty((0,))
    z_to_center = -torch.matmul(cameras.T, torch.transpose(cameras.R[0], 0, 1))[0, -1]
    ...
    if n_pts_per_ray > 0:
        ...
        z_to_foreground = -z_to_center+0.8
        z_foreg_to_back = 0.4

        # depths = torch.sqrt(depths)*(z_to_background+0.2)
